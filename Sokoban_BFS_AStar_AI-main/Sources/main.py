...
TIME_OUT = 900
''' nhận đường dẫn foler Testcases và Checkpoint '''
path_board = os.getcwd() + '\\..\\Testcases'
path_checkpoint = os.getcwd() + '\\..\\Checkpoints'

''' duyệt các tệp và trả lại 1 bảng '''
def get_boards():
    os.chdir(path_board)
    list_boards = []
...
''' từ các tệp đã kiểm tra và trả lại 1 tập hợp các điểm '''
...
''' format file txt kiểm tra đầu vào '''
def format_row(row):
...
#Trạng thái của màn gồm có chọn map và thuật toán, có màn loading, thực hiện thực toán.
sceneState = "init"
loading = False
	...
	while running:
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				...
				# mũi tên qua trái và phải để chọn map
				...
				if event.key == pygame.K_LEFT and sceneState == "init":
					if mapNumber > 0:
						mapNumber = mapNumber - 1
				# nhấn enter để thực hiện chương trình
				if event.key == pygame.K_RETURN:
					if sceneState == "init":
						sceneState = "loading"
					if sceneState == "end":
						sceneState = "init"
				#nhấn space để đổi thuật toán
...
def loadingGame():
	...
	text_1 = fontLoading_1.render('LOADINGGGGG!', True, WHITE)
	text_rect_1 = text_1.get_rect(center=(320, 60))
	screen.blit(text_1, text_rect_1)

	fontLoading_2 = pygame.font.Font('gameFont.ttf', 20)
	text_2 = fontLoading_2.render('The problem is being solved!!!', True, WHITE)
...
def foundGame(map):
	...
	text_1 = font_1.render('The problem is solved!!!', True, BLACK)
	text_rect_1 = text_1.get_rect(center=(320, 100))
	screen.blit(text_1, text_rect_1)

	font_2 = pygame.font.Font('gameFont.ttf', 20)
	text_2 = font_2.render('Press Enter to continue.', True, BLACK)
...
def notfoundGame():
	...
	text_1 = font_1.render('I tried my best :(((', True, WHITE)
	text_rect_1 = text_1.get_rect(center=(320, 100))
	screen.blit(text_1, text_rect_1)
